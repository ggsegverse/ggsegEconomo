...
* `economo` is now a `ggseg_atlas` object (from ggseg.formats) containing
  both 2D and 3D data. The separate `economo_3d` object has been removed.
...
* Atlas recreated from scratch using `ggsegExtra::create_cortical_atlas()`
  from the original fsaverage5 annotation files.
